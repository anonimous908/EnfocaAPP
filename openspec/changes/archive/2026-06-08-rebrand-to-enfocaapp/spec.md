# Delta for Visual Identity

## RENAMED Requirements

### Requirement: App Name String → "EnfocaApp"
(Reason: Rebrand from "Dopamina Guard")
(Migration: Update `app_name` in `strings.xml`)

### Requirement: Color Variable Prefix → "Enfoca"
(Reason: Align with new brand identity)
(Migration: Rename `Dopamina*` → `Enfoca*` in `Color.kt`; propagate to `Theme.kt`, `OnboardingContractScreen.kt`, `StatsScreen.kt`)

### Requirement: Code String Literals → "EnfocaApp"
(Reason: Remove legacy brand from source)
(Migration: Replace "Dopamina Guard" in `OnboardingContractScreen.kt` (2), `StatsScreen.kt` (1), `AGENTS.md` (1))

## MODIFIED Requirements

### Requirement: Welcome Screen Logo

The welcome screen logo MUST display at 128dp (was 64dp).
...
### Requirement: Legacy Scanner Overlay Inside Hub
(Reason: Replaced by standalone ring around logo)
(Migration: Remove old scanning composable from hub area; update any test references)
